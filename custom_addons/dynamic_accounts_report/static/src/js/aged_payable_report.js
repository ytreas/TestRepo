...
class AgedPayable extends owl.Component {
    async setup() {
        ...
        useEffect(
            ()=>{
                this.state.filter_range=`${this.state.end_date_bs?_t('Date:'):''} ${this.state.end_date_bs?this.state.end_date_bs:''}`
            },
            ()=>[this.state.end_date_bs]
        );
    ...
    init_date(){
        ...
        $('#end_date_ad').datepicker({
            ...
            onSelect: async function (dateText) {
                ...
                self.date_range.el.value=self.state.end_date_ad;
                let ev={
                    "target":{
                        "id":"end_date_ad",
                        "value":self.state.end_date_ad,
                        "attributes":""
                    }
                }
                self.applyFilter(ev,null);
    ...
    ev_mounted() {
        try{
            ...
            // var mainInput_ad = document.getElementById("end_date_ad");
            // var mainInput2 = document.getElementById("end_date");
            // var mainInput2_ad = document.getElementById("end_date_ad");
