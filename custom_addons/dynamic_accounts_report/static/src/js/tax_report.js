...
class TaxReport extends owl.Component {
    async setup() {
        ...
        this.end_date_bs = useRef('date_to_bs');
        this.start_date_bs = useRef('date_from_bs');
        ...
        setTimeout(async () => {
            if (e.target.id == "start_date") {
                if (this.state.start_date_bs != e.target.value) {
                    ...
                    this.start_date.el.value = this.state.start_date_ad
                    this.applyFilter(e, null);
                }
            }
            else {
                if (this.state.end_date_bs != e.target.value) {
                    ...
                    this.end_date.el.value = this.state.end_date_ad
    ...
    async get_date_bs(date) {
        let date_obj = await this.orm.call('account.general.ledger', 'ad_to_bs', [[date]]);

        return date_obj[0].date;
    ...
    async load_data() {
        ...
        try {
            ...
            self.start_date_bs.el.value = await this.get_date_bs(self.start_date.el.value);
            ...
            self.end_date_bs.el.value = await this.get_date_bs(self.end_date.el.value);
            self.state.date_viewed.push(monthNamesShort[today.getMonth()] + '  ' + today.getFullYear())
            self.state.date_viewed_np.push(monthNamesShort[today.getMonth()] + '  ' + today.getFullYear())
    ...
    async applyFilter(val, ev, is_delete) {
        ...
        if (val && val.target.name === 'start_date') {
            ...
            this.state.date_viewed_np = []
            this.state.date_viewed.push('From' + ' ' + this.formatDate(this.start_date.el.value) + ' ' + 'To' + ' ' + this.formatDate(this.end_date.el.value))
            this.state.date_viewed_np.push('From' + ' ' + this.formatDate(this.state.start_date_bs) + ' ' + 'To' + ' ' + this.formatDate(this.state.end_date_bs))
