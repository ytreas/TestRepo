...
class ProfitAndLoss extends owl.Component {
    ...
    async get_date_bs(date){
        let date_bs = await self.orm.call('account.general.ledger', 'ad_to_bs', [[dateText]]);

        return date_bs[0].date;
    ...
    async print_pdf(ev) {
        ...
        return self.action.doAction({
            ...
            'data': {
                'data': self.state,
                "start_date_bs":self.state.start_date_bs,
                "end_date_bs":self.state.end_date_bs,
                "start_date_ad":self.state.start_date_ad,
                "end_date_ad":self.state.end_date_ad,
