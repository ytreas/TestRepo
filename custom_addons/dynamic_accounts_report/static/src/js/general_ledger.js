    ...
    mounted() {
        try {
            ...
            mainInput.addEventListener('blur', (e) => {
                ...
                setTimeout(async () => {
                    if (self.state.start_date_bs != e.target.value) {
                        self.state.start_date_bs = e.target.value;
                        self.state.start_date_ad = await self.orm.call('account.general.ledger', 'bs_to_ad', [[e.target.value]]);
                        e.target.value = self.state.start_date_ad[0].date;
                        self.state.start_date_ad = self.state.start_date_ad[0].date;
                        self.applyFilter(e, null);
                    }
                }, 1000);

            });

            mainInput2.addEventListener('blur', (e) => {
                setTimeout(async () => {
                    if (self.state.end_date_bs != e.target.value) {
                        self.state.end_date_bs = e.target.value;
                        self.state.end_date_ad = await self.orm.call('account.general.ledger', 'bs_to_ad', [[e.target.value]]);
                        e.target.value = self.state.end_date_ad[0].date;
                        self.state.end_date_ad = self.state.end_date_ad[0].date;
