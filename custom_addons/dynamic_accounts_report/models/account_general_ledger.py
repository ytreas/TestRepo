...
class AccountGeneralLedger(models.TransientModel):
    ...
    def bs_to_ad(self,bs_date):
        try:
            date_string=''.join(bs_date)
            bs_year, bs_month, bs_day = map(int, date_string.split('-'))
            date=nepali_datetime.date(bs_year, bs_month, bs_day).to_datetime_date()
            print('type of bs date',type(bs_date))
            print('bs date value',bs_date)
            ad_date=[
                {
                    'date':date or '2081-07-23'
                }
            ]
            return ad_date
        except Exception as e:
            raise ValidationError(f"{_('Invalid date format')}- {e}")
    @api.model
    def ad_to_bs(self,ad_date):
      try:
        ...
        print('type of bs date',type(ad_date))
        print('bs date value',ad_date)
        ad_date=[
            {
                'date':bs_date or '2024-07-23'
            }
        ]
        return ad_date
      except Exception as e:
