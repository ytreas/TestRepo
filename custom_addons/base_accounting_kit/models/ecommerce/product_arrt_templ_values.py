...
class ProductTemplateAttributeValue(models.Model):
    ...
    _name = 'cp.template.attribute.value'
    ...
    attribute_line_id = fields.Many2one(
        comodel_name='cp.template.attribute.line',
        required=True, ondelete='cascade', index=True)
    ...
    currency_id = fields.Many2one(related='attribute_line_id.product_tmpl_id.product_id.currency_id')

    exclude_for = fields.One2many(
        comodel_name='cp.template.attribute.exclusion',
