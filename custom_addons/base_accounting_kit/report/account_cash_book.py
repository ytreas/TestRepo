...
class ReportCashBook(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby,
                                display_account, pass_date,form_data):
        ...
        ldate,l.date_bs AS date_bs, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
    ...
    def _get_report_values(self, docids, data=None):
        ...
        accounts_res = self.with_context(
            ...
            init_balance,
            sortby,
            display_account,data['form'].get('date_from'),  # Add pass_date parameter
            data['form']  )
        return {
            'doc_ids': docids,
            'doc_model': model,
