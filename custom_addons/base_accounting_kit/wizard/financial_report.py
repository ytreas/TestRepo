...
class FinancialReport(models.TransientModel):
    ...
    def _build_contexts(self, data):
        ...
        result['strict_range'] = True if result['date_from_bs'] else False
        return result
    ...
    date_from_bs = fields.Char(string='Start Date BS')
    date_to_bs = fields.Char(string='End Date BS')
    ...
    def view_report_pdf(self):
        ...
        data['form'] = self.read(
            ['date_from_bs', 'enable_filter', 'debit_credit', 'date_to_bs',
            ...
            if i['type'] == 'account':
                ...
                if form['target_move'] == 'posted':
                    ...
                    search_query = ("select aml.id, am.id as j_id, "
                                    ...
                                    "aml.account_id=%s")
                    vals = []
                if form['date_from_bs'] and form['date_to_bs']:
                    search_query += " and aml.date>=%s and aml.date<=%s"
                    vals += [account, form['date_from_bs'], form['date_to_bs']]
                elif form['date_from_bs']:
                    search_query += " and aml.date>=%s"
                    vals += [account, form['date_from_bs']]
                elif form['date_to_bs']:
                    search_query += " and aml.date<=%s"
                    vals += [account, form['date_to_bs']]
                else:
                    vals += [account]
