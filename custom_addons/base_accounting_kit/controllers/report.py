...
class AccountReport(http.Controller):
    ...
    @http.route('/trading/api/partner_ledger', type='http', auth='public', csrf=False, cors="*", methods=['POST'])
    def partner_ledger(self, **kw):
        """
        API to generate Partner Ledger Report (Receivable or Payable)
        Required POST data:
        {
            "date_from": "2025-04-25", # Analysis date (required)
            "date_to": "2025-05-30", # Analysis date (required)
            "result_selection": "customer", # "customer", "supplier", or "customer_supplier" (required)
            "target_move": "posted",    # "all" or "posted" (required)
            "company_id": 1,            # Company ID (default: from JWT token)
            "vendor_id": 1,           # Vendor ID (optional)    
            "journal_ids": [1,2,3]      # Journal IDs to include (optional, default: all)
        }
        """
        hosturl = request.httprequest.environ.get("HTTP_REFERER", "n/a")
        ...
        try:
            ...
            required_fields = ['date_from', 'date_to', 'result_selection', 'target_move']
            missing_fields = [field for field in required_fields if field not in data]
            if missing_fields:
                return request.make_response(
                    json.dumps({
            ...
            if vendor_id:
                ...
                return request.make_response(
                    json.dumps({
                        'status': 'fail',
                        'data': {'message': 'Vendor ID is required'}
                    }),
                    headers=[('Content-Type', 'application/json')],
                    status=400
                )
            ...
            # Create wizard and call check_report to get the report action
            print('wizard_vals:', wizard_vals)
            wizard = env['account.report.partner.ledger'].sudo().create(wizard_vals)
            report_action = wizard.check_report()
            print('report_action:', report_action)
            # 6. Generate the PDF using the report information from check_report
            report_name = report_action.get('report_name')
            if not report_name:
                report_name = 'base_accounting_kit.report_partnerledger'
            ...
            # Handle specific UserError exceptions from wizard validation
            _logger.warning(f"UserError in partner ledger report: {str(ue)}")
            return request.make_response(
                json.dumps({
                    'status': 'fail',
                    'data': {'message': str(ue)}
            ...
            _logger.exception("Error generating Partner Ledger report")
            return request.make_response(
                json.dumps({
                    'status': 'fail',
                    'data': {'message': f'An unexpected error occurred: {str(e)}'}
