...
class IZIAnalysis(models.Model):
        ...
        # Date Filters
            ...
            if kwargs.get('filters').get('date_format'):
                ...
                if date_format == 'custom' and kwargs.get('filters').get('date_range'):
                    ...
                    if start_date:
                        start_date_bs = datetime.strptime(start_date,"%Y-%m-%d").date()
                        start_date_nepali= nepali_datetime.date.from_datetime_date(start_date_bs)
                        start_final = start_date_nepali.strftime('%Y-%m-%d')
            
                        start_datetime = start_final

                        # start_datetime = start_date + ' 00:00:00'
                        # start_datetime = self.convert_to_utc(start_datetime) # Assuming this converts string to UTC datetime
                    if end_date:
                        end_date_bs = datetime.strptime(end_date,"%Y-%m-%d").date()
                        end_date_nepali= nepali_datetime.date.from_datetime_date(end_date_bs)
                        end_final = end_date_nepali.strftime('%Y-%m-%d')

                        end_datetime = end_final 

                        # end_datetime = end_date + ' 23:59:59'
                        # end_datetime = self.convert_to_utc(end_datetime)

                elif date_format != 'custom':
                    # Handle other date formats, such as 'today', 'week', 'month', etc.
                    print("*************************************",date_format)
                    date_range = self.get_date_range_by_date_format(date_format)  # Assuming this function calculates date ranges
                    ...
                    # end_datetime = self.convert_to_utc(end_datetime)
                # Create Domain using 'create_date' as datetime
                if start_datetime:
        
                    domain.append(('date_bs', '>=', start_datetime))

                if end_datetime:
                    domain.append(('date_bs', '<=', end_datetime))
            ...
            if kwargs.get('filters').get('date_format'):
                ...
                if date_format == 'custom' and kwargs.get('filters').get('date_range'):
                    ...
                    if start_date:
                        ...
                        test = datetime.strptime(start_date,"%Y-%m-%d").date()
                        test_nepali= nepali_datetime.date.from_datetime_date(test)
                        final = test_nepali.strftime('%Y-%m-%d')
                        start_datetime = final

                        # start_datetime = start_date + ' 00:00:00'
                        # start_datetime = self.convert_to_utc(start_datetime)  # Assuming this converts string to UTC datetime
                    if end_date:
                        test = datetime.strptime(end_date,"%Y-%m-%d").date()
                        test_nepali= nepali_datetime.date.from_datetime_date(test)
                        final = test_nepali.strftime('%Y-%m-%d')
                        end_datetime = final
                ...
                # Create Query using 'create_date' as datetime
                query_parts = []

                if start_datetime:
                    query = "date_bs >= '%s'" % start_datetime
                    # query = "create_date >= '%s'" % start_datetime
                    query_parts.append(query)

                if end_datetime:
                    query = "date_bs >= '%s'" % start_datetime
                    # query = "create_date <= '%s'" % end_datetime
                    query_parts.append(query)

                # Join query parts into a single filter query
                if query_parts:
                    final_query = " AND ".join(query_parts)
                    dashboard_filter_queries.append(final_query)
    ...
    def get_date_range_by_date_format(self, date_format):
        ...
        start_date_bs = datetime.strptime(start_date,"%Y-%m-%d").date()
        start_date_nepali= nepali_datetime.date.from_datetime_date(start_date_bs)
        final_start = start_date_nepali.strftime('%Y-%m-%d')
        start_datetime = final_start

        end_date_bs = datetime.strptime(end_date,"%Y-%m-%d").date()
        end_date_nepali= nepali_datetime.date.from_datetime_date(end_date_bs)
        final_end = end_date_nepali.strftime('%Y-%m-%d')
