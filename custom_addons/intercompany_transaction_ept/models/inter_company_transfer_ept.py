...
class InterCompanyTransferEpt(models.Model):
    ...
    source_warehouse_id = fields.Many2one("stock.warehouse", string="From Warehouse",
                                          help="Source warehouse to transfer stock from.", compute = "_get_source_warehouse_id", store= True)
    ...
    def _get_source_warehouse_id(self):
        for rec in self:
            if rec.src_company_id:
                warehouses = self.env['stock.warehouse'].sudo().search([('company_id','=',rec.src_company_id.id)])
                rec.source_warehouse_id = warehouses[0]
            else:
                rec.source_warehouse_id = False

    destination_warehouse_id = fields.Many2one("stock.warehouse", string="To Warehouse",
                                               help="Destination warehouse to transfer stock to.", compute = "_get_destination_warehouse_id", store= True)
    ...
    @api.depends('dst_company_id')
    def _get_destination_warehouse_id(self):
        for rec in self:
            if rec.dst_company_id:
                warehouses = self.env['stock.warehouse'].sudo().search([('company_id','=',rec.dst_company_id.id)])
                rec.destination_warehouse_id = warehouses[0]
            else:
    ...
    @api.constrains('src_company_id','dst_company_id')
    def _check_for_self_company(self):
        curr_companies = self._context['allowed_company_ids']
        print(curr_companies)
        for rec in self:
            if rec.src_company_id.id not in curr_companies and rec.dst_company_id.id not in curr_companies:
                raise UserError(_('One of the companies must be own company'))
    ...
    processed_date_bs = fields.Char(copy=False, help="Date when ICT is processed BS.")
    ...
    delivery_charge = fields.Float('Delivery Charge', compute = "_compute_delivery_charge")

    @api.depends('inter_company_transfer_line_ids')
    def _compute_delivery_charge(self):
        for rec in self:
            rec.delivery_charge = sum([rec.delivery_charge for rec in self.inter_company_transfer_line_ids])


    dest_allow = fields.Boolean(compute='_calc_dest_allow')

    _sql_constraints = [("source_destination_warehouse_unique",
                         "CHECK(source_warehouse_id != destination_warehouse_id)",
                         "Source and Destination warehouse must be different!")]
    
    # remarks = fields.Char("Remarks",size=500)
    @api.depends('dst_company_id')
    def _calc_dest_allow(self):
        print(self._context)
        for rec in self:
            if rec.dst_company_id and rec.dst_company_id.id in self.env.context['allowed_company_ids']:
                rec.dest_allow = False
            else:
                rec.dest_allow = True
    ...
    src_farmerid = fields.Char(
        string='Source Farmer ID'
    )
    ...
    @api.onchange('src_farmerid')
    def onchange_src_farmerid(self):
        if self.src_farmerid:
            farmer = self.env['res.company'].search([('farmerid', '=', self.src_farmerid)], limit=1)
            if farmer:
                self.src_company_id = farmer

    dst_farmerid = fields.Char(
        string='Destination Farmer ID'
    )
    dst_company_id = fields.Many2one('res.company',string=_('Destination company'))
    @api.onchange('dst_farmerid')
    def onchange_dst_farmerid(self):
        if self.dst_farmerid:
            farmer = self.env['res.company'].search([('farmerid', '=', self.dst_farmerid)], limit=1)
            if farmer:
                self.dst_company_id = farmer
    search_by_farmerid = fields.Boolean(string=_("Search By Farmer ID?"),default = False)            
    is_calculated_service_charge = fields.Boolean(string=_("Is calculated service charge"),default = False)
    calculated_service_charge = fields.Boolean(string=_("Already Calculated service charge"),default = False)
    corresponding_service_charge = fields.Many2one("inter.company.transfer.ept",string=_("Corresponding service charge"),default = False)
    ...
    nyear = fields.Char("Nepali Year")
    nmonth = fields.Char("Nepali Month")
    nday = fields.Char("Nepali Day")
    
    # @api.onchange('transaction_date')
    # def _calc_nepali_dates(self):
    #         english_date = parser.parse(str(self.transaction_date))
    #         print("--------------------------->",self.transaction_date)
    #         nepali_date_obj = nepali_date.from_datetime_date(english_date.date())
    #         print("nepali_date_obj",nepali_date_obj)
    #         self.nyear = nepali_date_obj.year
    #         print("year",self.nyear)
    #         self.nmonth = nepali_date_obj.month
    #         self.nday = nepali_date_obj.day
    ...
    def onchange_source_warehouse_id(self):
        ...
        if self.type != "internal":
            ...
            if self.source_company_id != self.destination_company_id:
                self.destination_warehouse_id = False
        temp = [("company_id", domain_operator, self.source_company_id.id),
                ("id", "!=", self.source_warehouse_id.id)]

        if self.dst_company_id:
            temp+=[('company_id','=',self.dst_company_id.id)]

        return {"domain": {"destination_warehouse_id": temp}}

    @api.onchange("dst_company_id")
    def onchange_source_warehouse_id(self):
        """
        Method will be executed when the value of source warehouse will be changed.
        @author: Maulik Barad.
    ...
    def calc_serv_charge(self):
        new_rec = self.new()
        
        old_source = self.src_company_id
        old_destination = self.dst_company_id

        old_source_wid = self.source_warehouse_id
        old_destination_wid = self.destination_warehouse_id

        new_rec.write({
            'src_company_id' : self.dst_company_id,
            'dst_company_id' : self.src_company_id,
            'auto_workflow_id' : self.auto_workflow_id,
            'currency_id' : self.currency_id,
            'is_calculated_service_charge' : True,
        })


        # new_rec.auto_workflow_id = self.auto_workflow_id

        total = sum([rec.service_charge for rec in self.inter_company_transfer_line_ids])

        charge = sum([rec.service_charge for rec in self.inter_company_transfer_line_ids])
        self.service_charge = charge
        product = self.env['product.product'].create({
            'name': f'{self.name} Service Charge',
            'uom_id': self.env.ref('uom.product_uom_unit').id,
            'lst_price': charge,
            'standard_price': charge,
            'type':'service'
        })

        product2 = self.env['product.product'].create({
            'name': f'{self.name} Delivery Charge',
            'uom_id': self.env.ref('uom.product_uom_unit').id,
            'lst_price': self.delivery_charge,
            'standard_price': self.delivery_charge,
            'type':'service'
        })

        service_line = self.env['inter.company.transfer.line.ept'].create({
        ...
        service_line2 = self.env['inter.company.transfer.line.ept'].create({
            'product_id':product2.id,
            'quantity':1,
            'delivered_qty':1,
            'price':self.delivery_charge

        })
        
        saved_rec=new_rec.copy()
        saved_rec.inter_company_transfer_line_ids+=service_line       
        saved_rec.inter_company_transfer_line_ids+=service_line2      
        self.calculated_service_charge = True
        self.corresponding_service_charge = saved_rec
        saved_rec.process_ict()
        print(self,new_rec,old_source,old_destination)
    ...
    def create_ict_sale_order(self):
        ...
        for record in self:
            ...
            if sale_order_lines:
                ...
                # sale_order.sudo()._generate_downpayment_invoices()
    ...
    def create_ict_purchase_order(self):
        ...
        for record in self:
            ...
            if purchase_order_lines:
                print('**********')
                print(order_vals)
                currency = self.env['res.company'].sudo().search([('id','=','1')])
                order_vals.update({
    ...
    def validate_ict_pickings(self, pickings):
        ...
        for picking in pickings.filtered(lambda x: x.state not in ["cancel", "done"]):
            print("Validating %s for %s." % (picking.name, self.name))
            ...
            if (self.type in ["ict", "internal"] and not self.auto_workflow_id.create_backorder) or (
                ...
                print("inside backorder")
                skip_backorder = True
                picking_ids_not_to_backorder = picking.id
            picking.move_ids_without_package.quantity = picking.move_ids_without_package.product_uom_qty
            print("above result")
            print("picking: ",picking)
            print("picking product: ",picking.product_id)
            print("picking product demand qty: ",picking.move_ids_without_package.product_uom_qty)
            print("picking product actual qty: ",picking.move_ids_without_package.quantity)
